...
class _UpdateSpaceState extends State<UpdateSpace> {
  ...
  void initState() {
    ...
    _validationMethod = widget.space.validation;

    if (widget.space.wifis != null) {
      for (var wifi in widget.space.wifis!) {
        wifiControllers.add({
          'name': TextEditingController(text: wifi.ssid),
          'password': TextEditingController(text: wifi.password),
        });
      }
    }
                    ...
                    InputForm(
                      ...
                      validator: MultiValidator([
                        ...
                        LengthRangeValidator(
                            min: 8, max: 12, errorText: AppLocalizations.of(context)!.phoneLengthError),
                    ...
                    Button(
                      ...
                      onPressed: () async {
                        if (_updateSpaceFormKey.currentState!.validate()) {
                          ...
                          widget.space.title = _titleSpaceController.text;
                          widget.space.phoneNumber = int.parse(_phoneNumberController.text);
                          widget.space.surfaceEnM2 = double.parse(_surfaceController.text);
                          widget.space.description = _descriptionController.text;
                          widget.space.validation = _validationMethod;

                          if (_validationMethod == 'wifi') {
                            widget.space.wifis = wifiControllers.map((wifiController) {
                              return WifiInfo(
                                ssid: wifiController['name']!.text,
                                password: wifiController['password']!.text,
                              );
                            }).toList();
                          } else {
                            widget.space.wifis = [];
